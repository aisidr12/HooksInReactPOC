...
export const InstragramApp = () => {
  ...
  const handleAddComment = async (formData:  FormData) => {
    ...
    addOptimisticComment(messageText); // Hace que el texto se muestre mientras que el texto se envia 

    //simula la peticion HTTP al servidor 
    await new Promise(resolve => setTimeout(resolve,3000));
    // es una manera de copiar los valores previos y crear un nuevo objeto 
      ...
      <ul className="flex flex-col items-start justify-center bg-gray-300 w-[500px] p-4">
        {optimisticComments.map((comment) => (
          <li key={comment.id} className="flex items-center gap-2 mb-2">
            ...
            { // si el optimistic es true hace que aparezca que se esta enviando
            comment.optimistic && (
